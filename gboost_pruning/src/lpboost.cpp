...
#define OPT " [-m minsup] [-p percent] [-x maxpat] [-n v] [-e conv_epsilon] [-r prune_ratio] [-o] graph-file"
#define ROW(i) ((i)+1)
#define COL(j) ((j)+1)
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "m:p:w:e:n:x:r:i")) != -1) {
    switch (opt) {
    case 'm':
      minsup = atoi (optarg);
      break;
  ...
  gspan.prune_ratio = prune_ratio;
  if(percent==true){
    gspan.minsup = gspan.gdata.size() * minp / 100;
  }
  std::cout << "prune_ratio:" << gspan.prune_ratio << std::endl;
  gspan.lpboost();
