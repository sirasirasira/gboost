...
void Gspan::edge_grow(GraphToTracers& g2tracers, int n) {
	...
	/*
	//debug
		for (auto fh = f_heap.begin(); fh != f_heap.end(); ++fh) {
			std::cout << "fh node " << fh->first << std::endl;
			for(auto fe = fh->second.begin(); fe != fh->second.end(); ++fe) {
				std::cout << "fe edge " << fe->first.a << "-" << fe->first.b << std::endl;
			}
		}
	*/
...
bool Gspan::make_count_b(unsigned int n, vector<int>& count, map<int, PairSorter, greater<int>>& b_heap, map<int, PairSorter, greater<int>>& f_heap, int maxtoc) {
	vector<double> probabilities(count.size(), 0.0);
	vector<double> bounds(count.size(), 0.0);
	unsigned int i = 0;
	bool prune = true;
