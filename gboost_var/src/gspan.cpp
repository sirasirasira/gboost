	...
	for (unsigned int j = 0; j < rm_path_index.size(); ++j) {
		...
		for (Tracers::iterator it =tracers.begin(); it != tracers.end(); ++it) {
			...
			for (unsigned int k=0; k<g[from_vpair.a].size(); ++k) {
				...
				if (minlabel > added_edge.labels.z || discovered[added_edge.to]) {
				   	continue; 
				}
...
Graph toGraph(vector<DFSCode>& codes) {
	...
	for (vector<DFSCode>::iterator p=codes.begin(); p!=codes.end(); ++p) {
		...
		if (p->labels.x != -1) {
		   	g.label[p->time.a] = p->labels.x;
		}
		if (p->labels.z != -1) {
		   	g.label[p->time.b] = p->labels.z;
		}
		edge.to = p->time.b;
...
int Gspan::scan_gspan(GraphToTracers& g2tracers, map<int,PairSorter,greater<int>>& b_heap, map<int,PairSorter,greater<int>>& f_heap) {
	scanum++;
	/*
	// build right most path
	   vector<int> rm_path_index;
...
void debugGraphToTracers(GraphToTracers g2tracers) {
	for (GraphToTracers::iterator g = g2tracers.begin(); g != g2tracers.end(); ++g) {
		std::cout << "gid : " << g->first << std::endl;
		for (auto _e = g->second.begin(); _e != g->second.end(); ++_e) {
			EdgeTracer *e = &(*_e);
			while (1) {
				std::cout << e->vpair.a << " : " << e->vpair.b << " - ";
				e = e->predec;
				if(e == 0) {
					break;
				}
