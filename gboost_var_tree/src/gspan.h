...
class Gspan {
 ...
 public:
  ...
  void make_count_s(unsigned int, vector<int>&, PairSorter&, map<int, PairSorter, greater<int>>&);
  bool make_count_b(unsigned int, vector<int>&, map<Triplet, GraphToTracers>&);
  bool make_count_b(unsigned int, vector<int>&, PairSorter&, map<int, PairSorter, greater<int>>&, int);
  double valuation();
